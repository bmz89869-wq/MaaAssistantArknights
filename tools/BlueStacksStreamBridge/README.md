...
The MAA integration starts this process only for the `BlueStacks` connection profile. It publishes a fixed `1280x720` BGRA frame, follows source orientation changes, and falls back to the existing ADB screenshot implementation whenever the map is missing, the producer heartbeat stalls, a post-input frame does not arrive, or the shared data is inconsistent.
...
The destination must contain the published executable files, `scrcpy-server-v4.1`, and `SCRCPY-LICENSE`. See `MAA_INTEGRATION.md` and `src/MaaCore/Controller/BlueStacksStreamBridge.*` for the lifecycle, ABI, and fallback behavior.
